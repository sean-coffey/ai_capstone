...
def get_data(data_dir):
    ...
    dates = [f"{years[i]}-{str(months[i]).zfill(2)}-{str(days[i]).zfill(2)}" for i in range(df.shape[0])]
...
def convert_to_ts(df_orig, country=None):
    ...
    start_month = f"{df['year'].values[0]}-{str(df['month'].values[0]).zfill(2)}"
    stop_month = f"{df['year'].values[-1]}-{str(df['month'].values[-1]).zfill(2)}"
...
if __name__ == "__main__":
    ...
    X, y, dates = engineer_features(ts_all['all'], training=True)